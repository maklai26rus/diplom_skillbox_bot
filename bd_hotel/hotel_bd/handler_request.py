...
SECRET_KEY_API = config('SECRET_KEY_API')
...
def request_processing_hotel(data):
    ...
    _en = 0  # нужный счечик, когда срабатывает ошибка KeyError ['ratePlan']['price']['current'] то оставляет значение
    for i in _hotel:
        try:
            _data_dict[_en] = [i['address']['locality'], i['name'], i['id'], i['landmarks'][0]['distance'],
